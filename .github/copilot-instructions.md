...
- **HA-version-bump bot uses rolling branches and runs daily.** [check-ha-version.yml](../.github/workflows/check-ha-version.yml) opens PRs on `ha-version-bump/stable` and `ha-version-bump/beta` (no version embedded) and runs on `cron: "0 6 * * *"`. Reject PRs that switch back to per-version branch names — that pattern accumulates one stale red PR per beta when bumps fail their gate. Reject PRs that drop the cron back to weekly.
