...
- **Version 0.1**:
  ...
  - Hardware-aware VOC entity creation — the VOC IAQ entity is now skipped at first registration on devices whose `hardware` string lacks the BME680/688 gas sensor (PA-I, original PA-II). PA-II-ZEN and newer (BME68X) get the entity as before. The gate fails closed on `hardware=None`: a transient missing-hardware response self-heals on the next setup, while failing open would permanently register an orphan entity on truly no-VOC devices that users would have to clean up manually. Pre-existing installs that already registered the entity for a no-VOC sensor keep it (the gate is bypassed when an entity with the matching unique_id is already in the registry). The `HARDWARE_GATES` map keeps adding gates for further `entity_registry_enabled_default=False` entities cheap because the gate transitively suppresses the description's `api_fields` (the coordinator's registry walk skips unregistered entities); entries that flip to enabled-by-default would also need a parallel gate in the coordinator's first-refresh fallback (`coordinator.py:_compute_requested_fields`), which iterates `SENSOR_DESCRIPTIONS` independently of the registry on the very first refresh.
