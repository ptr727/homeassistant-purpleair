...
- **Version 0.1**:
  ...
  - Hardware-aware VOC entity creation — the VOC IAQ entity is now skipped at first registration on devices whose `hardware` string lacks the BME680/688 gas sensor (PA-I, original PA-II). PA-II-ZEN and newer (BME68X) get the entity as before. The gate fails closed on `hardware=None`: a transient missing-hardware response self-heals on the next setup, while failing open would permanently register an orphan entity on truly no-VOC devices that users would have to clean up manually. Pre-existing installs that already registered the entity for a no-VOC sensor keep it (the gate is bypassed when an entity with the matching unique_id is already in the registry). The gate lives on `PurpleAirSensorEntityDescription.hardware_gate` so both `async_setup_entry` (entity creation) and `coordinator._compute_requested_fields` (first-refresh field selection) read from a single source of truth — adding a future hardware-gated entity is a one-line addition to its description, with no risk of the two code paths drifting out of sync.
