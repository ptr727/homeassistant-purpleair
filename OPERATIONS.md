...
- [`manifest.json`](./custom_components/purpleair/manifest.json) declares `ptr727-aiopurpleair` as a **minimum** (`>=`), not an exact pin. A new install gets the newest library release without an integration release, while HA leaves an existing install alone as long as it satisfies the minimum, so raising the minimum is the only way to move existing users forward. [`requirements-test.txt`](./requirements-test.txt) pins the exact version CI tests, and Dependabot bumps only that pin. `tests/components/purpleair/test_manifest.py` fails when the tested pin falls outside the manifest's requirement. Raise the manifest minimum by hand when the integration starts depending on a newer library release.
