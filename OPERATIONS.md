...
- [`manifest.json`](./custom_components/purpleair/manifest.json) declares `ptr727-aiopurpleair` as a **minimum** (`>=`), not an exact pin, so a library release reaches users without an integration release. [`requirements-test.txt`](./requirements-test.txt) pins the exact version CI tests, and Dependabot bumps only that pin. `tests/components/purpleair/test_manifest.py` fails when the tested pin falls outside the manifest's requirement. Raise the manifest minimum by hand when the integration starts depending on a newer library release.
