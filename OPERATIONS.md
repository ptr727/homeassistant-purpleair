...
pytest -ra                         # the test suite, inside the .venv
pylint custom_components/ tests/   # IDE-driven only, not a CI gate
```
...
- **Local state on disk** is limited to the `.venv`, the gitignored `config/` directory, and the gitignored `./aiopurpleair` checkout, none of which holds a credential.

## Local Rule Extensions

Rules that apply in this repository on top of the carried fleet rules. Each one is here because it is specific to this integration, not because it disagrees with the fleet.
...
[`publish-release.yml`](./.github/workflows/publish-release.yml) is **dispatch-only**, and **merges never publish**. HACS is a pull model that auto-pulls every new GitHub Release, so an automatic release on merge would force-update every user. The maintainer keeps both branches ready to release and ships on demand.
...
Each of these bit this repository at least once.

- **Multi-line `if` conditions use `if: >-`**, the folded scalar, not `if: |`. The folded form joins lines with single spaces, while the literal form keeps newlines the expression parser handles oddly.
- **Boolean inputs differ between `workflow_call` and `workflow_dispatch`.** `workflow_call` delivers real booleans and `workflow_dispatch` delivers the strings `"true"` and `"false"`, so an `if:` consuming one compares against both forms: `if: ${{ inputs.foo == true || inputs.foo == 'true' }}`. A bare `if: ${{ inputs.foo }}` reads `"false"` as truthy on the dispatch path.
- **Mirror inputs across both triggers** when a workflow supports `workflow_call` and `workflow_dispatch`. An input declared on only one side is `null` on the other.
