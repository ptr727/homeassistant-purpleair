...
- [`manifest.json`](./custom_components/purpleair/manifest.json) declares `ptr727-aiopurpleair` as a **minimum** (`>=`), not an exact pin. HA installs the newest release the minimum allows whenever the library is missing, which is a new install and, on a Docker-based install, every Home Assistant Core update, since the update replaces the container and the library with it. Otherwise HA leaves an installed library alone while it satisfies the minimum, so a venv install moves forward only when the minimum rises above what it has. [`requirements-test.txt`](./requirements-test.txt) pins the exact version CI tests, and Dependabot bumps only that pin. `tests/components/purpleair/test_manifest.py` fails when the tested pin falls outside the manifest's requirement. Raise the manifest minimum by hand when the integration starts depending on a newer library release.
