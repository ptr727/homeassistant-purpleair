...
# Dependabot secret store, not Actions secrets. CODECOV_TOKEN is Actions-only (coverage upload is not a
# Dependabot run); the HACS zip release is dispatch-gated and keyless, so there is no publish API key.
REQUIRED_ACTIONS_SECRETS=(CODEGEN_APP_CLIENT_ID CODEGEN_APP_PRIVATE_KEY CODECOV_TOKEN)
...
cmd_check() {
  ...
  check_secrets
  check_app
  note "verify manually: GitHub Releases / HACS zip publish is dispatch-gated; no external publish policy to verify."
