...
organization) secret store directly. The HACS zip release is cut by a dispatch-gated GitHub Release
(`publish-release.yml`), which is keyless - there is no external publish policy or API key for this script
to verify.
...
```sh
REPO=ptr727/HomeAssistant-PurpleAir ./repo-config/configure.sh apply   # sync live rulesets + settings + security
REPO=ptr727/HomeAssistant-PurpleAir ./repo-config/configure.sh check   # confirm no drift
```
